...
**Card icons come from `Chart.yaml` only.** Every card on the site renders the icon its
chart declares — there are no icon overrides anywhere.

Descriptions are the one exception: `docs/src/js/display.js` still carries a per-chart
`description` (and a `name`, so brand names read as `ClickHouse` rather than `Clickhouse`),
and an entry there wins over `Chart.yaml`. It currently holds a description for every card
on the site, so if you change a `description` and nothing appears on the site, look there
first. It never affects *whether* a chart appears or which category it lands in — that is
decided by `annotations.type` alone — and a chart with no entry renders straight from its
own `Chart.yaml`.
