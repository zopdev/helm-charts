...
| `icon` | The card image, unless the chart has an icon override in `docs/src/js/display.js` (see below). Charts without either fall back to the zop.dev logo. |
...
below) — the site reads released chart metadata, not `charts/`, so editing `Chart.yaml`
alone changes nothing on the site. Bump the version when you change an existing chart's
`icon` or `description`: re-releasing under the same version does reach `index.yaml` when
the index is regenerated by hand, but the release workflow skips versions it has already
published.

**An entry in `docs/src/js/display.js` takes precedence over `Chart.yaml`.** That table
currently overrides the description of every card on the site and the icon of 18 of the
22, so for those charts editing `Chart.yaml`'s `icon` or `description` has no visible
effect until the override is removed — if you change one and nothing happens, look there
first. It never affects *whether* a chart appears or which category it lands in; that is
decided by `annotations.type` alone, and a chart with no entry in the table renders
straight from its own `Chart.yaml`.

To preview the site locally, serve the folder (`cd docs && python3 -m http.server 8000`).
Opening `index.html` straight from disk does not work: the browser blocks the page's ES
modules on `file://`, so the grid stays empty with no error shown.
